...
void readoutdata(char const * inputfile, char const * histname, char const * regionfile){
  ...
  ///////Add to read the global mean and sigma from the original root file
  string OrigMeanFile="Original_Global_Value.txt";
  ifstream origfile;
  string Oname;
  double Omean;
  double Osigma;
  origfile.open (OrigMeanFile.c_str());
  if (origfile.is_open())
    {
      while(!origfile.eof())
	{
	  ...
	  double origrms;
	  double origsigma;
	  getline (origfile,wholeline);
	  std::stringstream ss(wholeline);
	  ss >> orighistname >> origrunnums >> origentries >> origmean >> origrms >> origsigma;
	  if(orighistname==histname){
	    //	    cout << "Name: " << orighistname << "; Mean:" << origmean << "; Sigma:" << origsigma <<'\n';
	    Oname=orighistname;
	    Omean=origmean;
	    Osigma=origsigma;
	  }
  ...
  if( Oname != histname){
    cout << "NOTICE!!!!!! The hist name is not right, please check" << '\n';
  }
  //Here we use the orignal one replace with the new one
  //  double xmean=myHist->GetMean(2);
  //  double xsigma=myHist->GetMeanError(2)*sqrt(goodrunno);
  double xmean=Omean;
  double xsigma=Osigma;
  ...
  ///debug
  //  cout << xlowlimit << ' ' << xhighlimit <<  endl;
  ...
  for(int jj=0;jj<(j-1);jj++){
    ...
    for(int i=1;i<=myHist->GetNbinsX();i++){
      if(jj!=j-2){
	if(myHist->GetBinCenter(i)>=cutids[jj]&&myHist->GetBinCenter(i)<cutids[jj+1]){
	  if((myHist->GetBinContent(i)!=0)&&(myHist->GetBinError(i)!=0)){
	    h1->Fill(myHist->GetBinContent(i), 1./(myHist->GetBinError(i))/(myHist->GetBinError(i)));
	    counter++;
	  }
	}
      }else{
   ...
   //   for(int i=1;i<myHist->GetNbinsX();i++){if(myHist->GetBinCenter(i)>=cutids[jj]&&myHist->GetBinCenter(i)<cutids[jj+1]){if(myHist->GetBinContent(i)!=0){fprintf(stdout,"%ld %g %g %g %g %d %g \n",long(myHist->GetBinCenter(i)),myHist->GetBinContent(i),myHist->GetBinError(i),meanh1,rmsh1,goodrunno2,myHist->GetMeanError(2)*sqrt(goodrunno)); goodrunno2++;}}};
    ...
    //Replace with the orig mean and sigma, basically same except the last number
      if(jj!=j-2){
      for(int i=1;i<=myHist->GetNbinsX();i++){if(myHist->GetBinCenter(i)>=cutids[jj]&&myHist->GetBinCenter(i)<cutids[jj+1]){if(myHist->GetBinContent(i)!=0){fprintf(stdout,"%ld %g %g %g %g %d %g \n",long(myHist->GetBinCenter(i)),myHist->GetBinContent(i),myHist->GetBinError(i),meanh1,rmsh1,goodrunno2,Osigma); goodrunno2++;}}};
      }else{
	for(int i=1;i<=myHist->GetNbinsX();i++){if(myHist->GetBinCenter(i)>=cutids[jj]&&myHist->GetBinCenter(i)<=cutids[jj+1]){if(myHist->GetBinContent(i)!=0){fprintf(stdout,"%ld %g %g %g %g %d %g \n",long(myHist->GetBinCenter(i)),myHist->GetBinContent(i),myHist->GetBinError(i),meanh1,rmsh1,goodrunno2,Osigma); goodrunno2++;}}};
      }
      h1->Reset();    
