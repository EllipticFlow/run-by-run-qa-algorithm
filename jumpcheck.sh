...
##To avoid the GetMean are different in the different cases, we will calculate the mean by ourselves anyway(update on Sep,03,2020)
if [ ${nobadruns} -eq 1 ]
then
    cat ${input}_raw.dat | awk '{if ($1 == ($1+0)) print $0}' | awk '{print $0, NR}' | sort -n -k 8,8 >"file_Goodruncut_sort_old"
else
...
#exit;
	...
	do
	    ...
	    #	    echo "LOOK here:" ${lowlimit_1} ${peakline_1} ${highlimit_1}
...
#	    awk '{if ($3!=0){ u += $2*(1/($3*$3)); w += (1/($3*$3)); m = u/w; s += (1/($3*$3)); sig = (1/s)^0.5 }} END {print '$tempnumber',m, sig*'$runnb'^0.5}' "rawpeak_tempregion_${tempnumber}.dat" >> local_mean_sigma.txt 
	...
	#Fuqiang suggests to remove 1% and check 2-RMS(will check in runbyrun_advanced.sh)
	awk '{ n1=n2; u1=u2; s1=s2; u2=$2; s2=$3; n2=$1 } {if( n1!=n2 ) {if( (NR>1)&&((u1-u2)^2-'$nsigma'*'$nsigma'*(s1^2+s2^2) >= 0.0 )&&(((u1-u2)/u1)^2 >= 0)) print( n1, n2, (u1-u2)^2, (s1^2+s2^2), (u1-u2)^2-'$nsigma'*'$nsigma'*(s1^2+s2^2), ((u1-u2)/u1)^2)}}' "local_mean_sigma.txt" > jump_in_tempregion.txt #give the jump place between two region numbers
	...
	mv Cut_for_${input}.dat temp_cut_for_${input}.dat
	cat temp_cut_for_${input}.dat | awk '{if($1+0==$1){print($0)}}' > Cut_for_${input}.dat 
